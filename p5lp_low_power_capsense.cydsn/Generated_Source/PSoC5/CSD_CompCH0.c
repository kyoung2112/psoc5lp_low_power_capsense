...
uint8 CSD_CompCH0_initVar = 0u;

/* Internal functions definitoin */
static void CSD_CompCH0_trimAdjust(uint8 nibble) ;

/* static CSD_CompCH0_backupStruct  CSD_CompCH0_backup; */
#if (CY_PSOC5A)
    static CSD_CompCH0_LOWPOWER_BACKUP_STRUCT  CSD_CompCH0_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 CSD_CompCH0_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: CSD_CompCH0_Init
********************************************************************************
*
* Summary:
...
void CSD_CompCH0_Init(void) 
{
    ...
    CSD_CompCH0_SetSpeed(CSD_CompCH0_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( CSD_CompCH0_DEFAULT_HYSTERESIS == 0u )
        CSD_CompCH0_CR |= CSD_CompCH0_HYST_OFF;
    #else
        CSD_CompCH0_CR &= (uint8)(~CSD_CompCH0_HYST_OFF);
    #endif /* CSD_CompCH0_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( CSD_CompCH0_DEFAULT_PWRDWN_OVRD == 0u )
            CSD_CompCH0_CR &= (uint8)(~CSD_CompCH0_PWRDWN_OVRD);
        #else 
            CSD_CompCH0_CR |= CSD_CompCH0_PWRDWN_OVRD;
        #endif /* CSD_CompCH0_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    CSD_CompCH0_CR |= CSD_CompCH0_MX_AO;

    /* Set default sync */
    CSD_CompCH0_CLK &= (uint8)(~CSD_CompCH0_SYNCCLK_MASK);
    #if ( CSD_CompCH0_DEFAULT_BYPASS_SYNC == 0u )
        CSD_CompCH0_CLK |= CSD_CompCH0_SYNC_CLK_EN;
    #else
        CSD_CompCH0_CLK |= CSD_CompCH0_BYPASS_SYNC;
    #endif /* CSD_CompCH0_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: CSD_CompCH0_Enable
********************************************************************************
*
* Summary:
...
{
    CSD_CompCH0_PWRMGR |= CSD_CompCH0_ACT_PWR_EN;
    CSD_CompCH0_STBY_PWRMGR |= CSD_CompCH0_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(CSD_CompCH0_restoreReg == 1u)
        {
            CSD_CompCH0_CR = CSD_CompCH0_lowPowerBackup.compCRReg;

            /* Clear the flag */
            CSD_CompCH0_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: CSD_CompCH0_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  CSD_CompCH0_initVar: Is modified when this function is called for the 
...
void CSD_CompCH0_Start(void) 
{
    ...
    if ( CSD_CompCH0_initVar == 0u )
    {
        CSD_CompCH0_Init();
        
        CSD_CompCH0_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: CSD_CompCH0_Stop
********************************************************************************
*
* Summary:
...
void CSD_CompCH0_Stop(void) 
{
    ...
    CSD_CompCH0_PWRMGR &= (uint8)(~CSD_CompCH0_ACT_PWR_EN);
    CSD_CompCH0_STBY_PWRMGR &= (uint8)(~CSD_CompCH0_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        CSD_CompCH0_restoreReg = 1u;

        /* Save the control register before clearing it */
        CSD_CompCH0_lowPowerBackup.compCRReg = CSD_CompCH0_CR;
        CSD_CompCH0_CR = CSD_CompCH0_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: CSD_CompCH0_SetSpeed
********************************************************************************
*
* Summary:
...
void CSD_CompCH0_SetSpeed(uint8 speed) 
{
    ...
    CSD_CompCH0_CR = (CSD_CompCH0_CR & (uint8)(~CSD_CompCH0_PWR_MODE_MASK)) |
                           (speed & CSD_CompCH0_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == CSD_CompCH0_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            CSD_CompCH0_TR = CSD_CompCH0_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CSD_CompCH0_TR0 = CSD_CompCH0_HS_TRIM_TR0;
            CSD_CompCH0_TR1 = CSD_CompCH0_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            CSD_CompCH0_TR = CSD_CompCH0_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            CSD_CompCH0_TR0 = CSD_CompCH0_LS_TRIM_TR0;
            CSD_CompCH0_TR1 = CSD_CompCH0_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: CSD_CompCH0_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( CSD_CompCH0_WRK & CSD_CompCH0_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: CSD_CompCH0_trimAdjust
********************************************************************************
*
* Summary:
...
static void CSD_CompCH0_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = CSD_CompCH0_WRK & CSD_CompCH0_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CSD_CompCH0_TR |= CSD_CompCH0_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CSD_CompCH0_TR0 |= CSD_CompCH0_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                CSD_CompCH0_TR |= CSD_CompCH0_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                CSD_CompCH0_TR1 |= CSD_CompCH0_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((CSD_CompCH0_WRK & CSD_CompCH0_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: CSD_CompCH0_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 CSD_CompCH0_ZeroCal(void) 
{
    ...
    tmpSW0 = CSD_CompCH0_SW0;
    tmpSW2 = CSD_CompCH0_SW2;
    tmpSW3 = CSD_CompCH0_SW3;

     /* Clear routing for inP, retain routing for inN */
    CSD_CompCH0_SW0 = 0x00u;
    CSD_CompCH0_SW2 = 0x00u;
    CSD_CompCH0_SW3 = tmpSW3 & (uint8)(~CSD_CompCH0_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = CSD_CompCH0_CR;
    CSD_CompCH0_CR |= (CSD_CompCH0_CAL_ON | CSD_CompCH0_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        CSD_CompCH0_TR = CSD_CompCH0_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        CSD_CompCH0_TR0 = CSD_CompCH0_DEFAULT_CMP_TRIM;
        CSD_CompCH0_TR1 = CSD_CompCH0_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (CSD_CompCH0_CR & CSD_CompCH0_PWR_MODE_MASK) == CSD_CompCH0_PWR_MODE_FAST)
    {
        CSD_CompCH0_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        CSD_CompCH0_trimAdjust(1u);
		CSD_CompCH0_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    CSD_CompCH0_CR = tmpCR;
    
    /* Restore routing registers for inP */
    CSD_CompCH0_SW0 = tmpSW0;
    CSD_CompCH0_SW2 = tmpSW2;
    CSD_CompCH0_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) CSD_CompCH0_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)CSD_CompCH0_TR1 << 8u) | (CSD_CompCH0_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: CSD_CompCH0_LoadTrim
********************************************************************************
*
* Summary:
...
void CSD_CompCH0_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: CSD_CompCH0_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: CSD_CompCH0_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
